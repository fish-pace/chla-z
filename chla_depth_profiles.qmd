...
# 🌍 Why Global CHLA Depth Profiles?

- Surface chlorophyll maps are abundant — but **ecosystems live in the water column**, not just the surface.
- Vertical structure of CHLA reveals:
  - **Subsurface chlorophyll maxima**
  - Light–nutrient tradeoffs
  - Mixed-layer dynamics
- Yet:  
  **We have no global product providing full CHLA-depth profiles daily.**
...
# 🌊 Why Do Depth Profiles Matter (Fisheries)?

- Many fish species interact with **subsurface productivity layers**, not just the surface.
- Larval fish and forage fish rely on **subsurface prey fields** governed by CHLA depth structure.
- Vertical structure informs:
  - Habitat models
  - Recruitment studies
  - Climate change impacts on lower trophic levels

**Depth ≠ just a detail — it's the environment fish actually occupy.**

---

# 🌫️ Why CHLA Profiles Matter (Carbon Cycling)

- Phytoplankton depth distribution controls:
  - **Primary production**
  - **Carbon export efficiency**
  - **Light absorption and attenuation**
- Subsurface CHLA maxima can dominate **net community production**.

**Accurate vertical biomass is essential for carbon-cycle models.**

---

# 🌈 Why PACE Changes the Game

- PACE’s hyperspectral **Rrs(λ)** captures more spectral detail than prior missions.
- It samples biological and optical features that:
  - Penetrate below the surface
  - Respond to pigment composition
  - Vary with particle concentration and size
- With hyperspectral data, we can infer **signals tied to subsurface structure**, not just surface color.

---

# 📈 What Does Hyperspectral Rrs Look Like?

```{python}
import numpy as np
import matplotlib.pyplot as plt

wavelength = np.arange(350, 800, 2)
rrs = 0.005 + 0.003 * np.exp(-(wavelength - 440) ** 2 / 4000) + 0.001 * np.sin(wavelength / 40)

plt.figure(figsize=(6, 4))
plt.plot(wavelength, rrs)
plt.xlabel("Wavelength (nm)")
plt.ylabel("Rrs")
plt.title("Example Hyperspectral Rrs Spectrum")
plt.tight_layout()
plt.show()
```
...
# 🤖 In-situ Platforms: Bio-Argo

- Profiling floats: 0–1000+ m  
...
# 🗃️ Matching Step: Match in-situ lat/lon/day to PACE Level 3 daily data
...
# 🌍 Where Are the Training Profiles?
...
Perfect for mapping **hyperspectral Rrs → scalar CHLA at a given depth**.
...
- Input:  
  ...
  - Extras (solar hour, platform type, etc.)
- Output:  
  - CHLA in that bin (e.g., 0–10 m, 10–20 m, …)

Benefits:

- Uses **partial profiles** (only bins with valid CHLA).
...
Overall we see a close match between our preditions and our test data for CHLA in the upper 10m.
...
Using PACE data for hyperspectral Rrs, we can make a prediction of surface CHLA. Note it is not our goal to predict surface CHLA as there are many existing surface CHLA products. We will be predicting CHLA depth profiles 0 to 200m. But let's look at this first.

Let's compare to PACE's surface CHLA product as a first pass check, but note that these are different products trained on different in-situ data.
...
# 👁️ Example CHLA Profiles: Observed vs BRT
...
# 📉 Integrated CHLA (0–200 m): Obs vs Pred
...
# 🗺️ Toward Global Maps of CHLA Profiles
...
This moves us toward **global, depth-resolved phytoplankton structure from space**.
