---
title: Revealing the Hidden Phytoplankton Layer below the Surface
subtitle: Satellite-Driven Estimates of Global Chlorophyll Depth Profiles using PACE Hyperspectral Ocean Color Data
author: Eli Holmes
format:
  revealjs:
    theme:
...
# Example: Seasonal pattern of peak
...
```{python}
#| fig-align: center
#| fig-cap: Vertical lines indicate representative band centers of Sentinel-3 OLCI, shown for comparison with hyperspectral PACE OCI observations.
#| fig-height: 4
#| fig-width: 8
...
# --- 1) Fetch one PACE OCI L3M RRS granule ---
...
# --- 2) Pick a point and extract spectrum ---
lat0, lon0 = 50, -60
rrs = ds["Rrs"].sel(lat=lat0, lon=lon0, method="nearest")

# Find wavelength dimension robustly
wl_dim = next((d for d in rrs.dims if d.lower() in ("wavelength", "wl", "lambda", "band")), rrs.dims[0])
wl = rrs[wl_dim].values

# --- 3) Multispectral band centers (nm) ---
# MODIS (Aqua/Terra) common ocean-color bands (approx centers)
modis_nm = [412, 443, 469, 488, 531, 547, 555, 645, 667, 678]

# Sentinel-3 OLCI band centers (exact-ish; standard published centers)
olci_nm = [400, 412.5, 442.5, 490, 510, 560, 620, 665, 673.75, 681.25,
           708.75, 753.75, 778.75, 865, 885]

# --- 4) Plot ---
fig, ax = plt.subplots(figsize=(9, 4))

ax.plot(wl, rrs.values, linewidth=2)
ax.set_title(
    f"PACE OCI L3M Rrs spectrum near lat={float(rrs['lat'].values):.2f}, "
    f"lon={float(rrs['lon'].values):.2f}\n"
    "Sentinel-3 OLCI band centers shown"
)
ax.set_xlabel("Wavelength (nm)")
ax.set_ylabel("Rrs")

# Thicker band-center lines with distinct colors
# ax.vlines(modis_nm, *ax.get_ylim(), linewidth=2.5, alpha=0.45, color="tab:blue",   label="MODIS")
ax.vlines(olci_nm,  *ax.get_ylim(), linewidth=2.5, alpha=0.45, color="tab:purple", label="Sentinel-3 OCLI")
...
# Load data
url = "https://raw.githubusercontent.com/fish-pace/fish-pace-datasets/main/datasets/chla_z/data/CHLA_argo_profiles.parquet"
...
import pandas as pd
url = "https://raw.githubusercontent.com/fish-pace/fish-pace-datasets/main/datasets/chla_z/data/argo_na_2024_03.parquet"
...
# Add a shaded layer representing "surface CHLA" (e.g. 0–10 m)
...
# Load data
url = "https://raw.githubusercontent.com/fish-pace/fish-pace-datasets/main/datasets/chla_z/data/CHLA_ooi_profiles.parquet"
df = pd.read_parquet(url)
...
# keep original true values in linear space
y_test_linear = y_test.copy()

# transform for the scatter plot
y_test = np.log10(y_test)

# valid mask for log plot / paired values
valid = y_test.notna() & np.isfinite(y_test) & np.isfinite(y_pred)
...
# convert predictions back to linear space for error histogram
true_linear = y_test_linear[valid]
pred_linear = 10 ** y_pred

# linear errors
err_linear = true_linear - pred_linear

fig, axes = plt.subplots(1, 2, figsize=(10, 5))

# 1) existing scatter plot in log space
ax = axes[0]
...
ax.set_xlabel("log10 Chl-a (true)")
ax.set_ylabel("log10 Chl-a (prediction)")
ax.set_title(f"Prediction vs Truth\nR² = {r2:.3f}, RMSE = {rmse:.3f}")

# 2) histogram of linear errors
ax = axes[1]
ax.hist(err_linear, bins=40)
ax.axvline(0, linestyle="--")

ax.set_xlabel("Error (true − pred, linear Chl-a)")
ax.set_title("Distribution of linear errors")

mean_err = np.mean(err_linear)
median_err = np.median(err_linear)
...
* CHLA at each depth bin
* Depth of peak CHLA
* Height of peak CHLA
* Integrated CHLA from 0 to 200m

# CHLA Profiles: Observed vs BRT  { .nostretch }
...
scale = 0.75  # try 0.7 or 0.65 if needed
fig, axes = plt.subplots(nrows, ncols, figsize=(scale * 3 * ncols, scale * 4 * nrows), sharey=True)
...
# When surface CHLA is high?  { .nostretch }
...
```{python}
# functions to compute Peak Depth, Int CHLA and Metrics
import numpy as np
import pandas as pd
from scipy.ndimage import uniform_filter1d
...
# Observed and Predicted Peak Depth
