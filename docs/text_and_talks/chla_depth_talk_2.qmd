...
valid = y_test.notna() & np.isfinite(y_test) & (y_test > 0) & np.isfinite(y_pred)
...
# linear errors
err_linear = pred_linear - true_linear
...
ax.set_xlabel("Error (pred − true, linear Chl-a)")
...
```{python}
# code for metrics is above where model loaded
```

# Absolute Errors for CHLA in Depth Bins
